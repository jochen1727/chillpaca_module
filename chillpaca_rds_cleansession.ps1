...
#>
    ...
    if ($killprocessus) {
        $ServeursRDS = Get-RDServer -ConnectionBroker $serveurbrokerrds
    ...
    foreach ($server in $serveursrds) {
        Invoke-Command -ComputerName $server -ScriptBlock {
            Get-ChildItem -Path "C:\ProgramData\Microsoft\Group Policy\Users\" | Remove-Item -Force -Recurse
            Get-ChildItem -Path 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Group Policy\DataStore' -Exclude M* | Remove-Item -Force -Recurse
            <#
